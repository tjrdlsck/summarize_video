...
class VideoClipper:
    ...
    def _merge_subtitles(self, original_sub_path, segments, output_sub_path):
        """
        여러 구간의 자막을 추출하여 하나의 타임라인(0초 시작)으로 병합합니다.
        """
        if not os.path.exists(original_sub_path): return None
        
        # 원본 자막 파싱 (cut_subtitle 로직 활용을 위해 전체 로드 방식 사용 안 함)
        # 대신, 각 세그먼트별로 cut_subtitle을 호출하고 시간을 Shift하는 방식 사용
        
        merged_lines = []
        current_offset = 0.0 # 병합된 타임라인에서의 현재 위치
        index_counter = 1
        
        try:
            for seg in segments:
                # 1. 해당 구간의 자막을 임시 파일로 추출
                temp_sub_name = f"temp_sub_{uuid.uuid4().hex[:8]}.srt"
                temp_sub_path = self.cut_subtitle(original_sub_path, seg['start'], seg['end'], output_filename=temp_sub_name)
                
                if temp_sub_path and os.path.exists(temp_sub_path):
                    with open(temp_sub_path, 'r', encoding='utf-8') as f:
                        lines = f.readlines()
                    
                    # 2. 추출된 자막의 타임스탬프를 current_offset 만큼 뒤로 밀기
                    # cut_subtitle은 0초부터 시작하게 만들어주므로, 여기에 offset만 더하면 됨
                    time_pattern = re.compile(r'(\d{2}:\d{2}:\d{2},\d{3})\s-->\s(\d{2}:\d{2}:\d{2},\d{3})')
                    ...
                    for line in lines:
                        if '-->' in line:
                            match = time_pattern.search(line)
                            if match:
                                t1 = self._time_str_to_seconds(match.group(1).replace(',', '.'))
                                t2 = self._time_str_to_seconds(match.group(2).replace(',', '.'))
